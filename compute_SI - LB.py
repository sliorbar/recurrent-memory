...
#   infloss_test = data['frac_rmse_test']    
    ...
    if (infloss_test<infloss_threshold):
        ...
        for b in range(bs):
            hidr_t = hidr[b,:,:]
            #Select in each trial the responses of all the neurons were activated above threshold
            selected_indx = np.nonzero(np.mean(hidr_t,axis=0)>r_threshold)[0]
            hidr_t = hidr_t[:,selected_indx]#hidr_t= hiden response time
            ...
            etr=np.histogram(peak_times, entrpy_bins)[0]+0.1*np.ones(entrpy_bins)
            entrpy = entropy(etr)
            #entrpy = entropy(np.histogram(peak_times, entrpy_bins)[0]+0.1*np.ones(entrpy_bins))
            #Lior - Added code
           #entrpy = entropy(np.histogram(peak_times, entrpy_bins)+0.1*np.ones(entrpy_bins))
        ...
        print ('Mean SI {}, {:f}'.format(j,SI_job_vec[j]))
