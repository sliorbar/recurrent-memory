...
def LeInitRecurrentWithLayerNorm(input_var, mask_var=None, batch_size=1, n_in=100, n_out=1,
    ...
    l_hid_hid = DenseLayer(lasagne.layers.InputLayer((None, n_hid)), n_hid,
                           W=LeInit(diag_val=diag_val, offdiag_val=offdiag_val),
                           nonlinearity=lasagne.nonlinearities.linear)
    l_rec = RecurrentNormalizingLayer(l_in, l_in_hid, l_hid_hid, n_hid, mask_input=l_mask, grad_clipping=100, hid_init=lasagne.init.Uniform([0.0,0.1]))
...
def ResidualRecurrent(input_var, mask_var=None, batch_size=1, n_in=100, n_out=1, n_hid=200, leak_inp=1.0, leak_hid=1.0, out_nlin=lasagne.nonlinearities.linear):
    # Input Layer
    l_in         = InputLayer((batch_size, None, n_in), input_var=input_var)
    if mask_var==None:
        l_mask=None
    ...
    l_in_hid     = DenseLayer(lasagne.layers.InputLayer((None, n_in)), n_hid,  W=lasagne.init.GlorotNormal(0.1), b=None, nonlinearity=lasagne.nonlinearities.linear)
    l_hid_hid    = DenseLayer(lasagne.layers.InputLayer((None, n_hid)), n_hid, W=lasagne.init.GlorotNormal(0.1), nonlinearity=lasagne.nonlinearities.linear)
    l_rec        = ResidualCustomRecurrentLayer(l_in, l_in_hid, l_hid_hid, nonlinearity=lasagne.nonlinearities.rectify, mask_input=l_mask, grad_clipping=100, leak_inp=1.0, leak_hid=1.0)
